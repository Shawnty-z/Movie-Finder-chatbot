...
function generateMovieResponse(movie) {
    ...
    let response = `🎬 <strong>${movie.title}</strong> (${year})`;
    if (rating) {
        response += `<br>${rating}`;
    }
    if (movie.overview) {
        response += `<br>${movie.overview}`;
    } else {
        response += `<br>A great movie in this genre! Unfortunately, I don't have a description available.`;
    }
    return response;
}
